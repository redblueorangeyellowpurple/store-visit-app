...
## Routes
- `/` — **Reports tab** (landing), three top-level tabs **Daily | Weekly | Memories**: **Daily** = the intelligence brief — exec stat-tiles + CM table + 🌟 Good News + Signals/Alerts + 🤝 Engagements cards (parses stored `brief_markdown`; new nested-bullet format AND legacy flat format both render; legacy 🧵 Threads sections fold into the Signals card — Signals = good/neutral patterns + notable one-offs, Alerts = bad/needs attention), day-nav (`‹ ›` arrows + date dropdown + Latest + `?date=` deep-link). **Weekly** (`WeeklyView` + week-nav): deterministic sections (stat cards incl. Executed/Planned + engagement headline row, Visits By Day, Execution Summary with three switchable views (segmented control) — Channel Managers table (per-CM `CMEngagementsDrawer`) / Products table (per-product `TrainingProductDrawer`) / Stores per-day amber heatmap (`storeDayMatrix`, counts in cells + week-total column, Country/Tier outer grouping over always-by-chain groups, chains collapsed by default, store name → store drawer)) computed live by `lib/weekly.ts`; AI narrative (Good News/Signals/Alerts/Engagements, single emoji + inline kicker beside the title) read from `sva.weekly_reports` (mig 028) or a placeholder; **Store Updates sits at the bottom** — group-by chips (Market/Chain/Tier) over 1-col display-only store cards → `StorePhotosDrawer` (photos grouped by week via `GET /api/stores/[id]/photos`, per-photo comments/annotations reuse `FeedPhotoLightbox` + the existing `/api/photos/[id]/*` routes, so comments flow into the CM daily recap). Defaults to the last completed Mon–Sun week. Routes: `GET /api/intelligence/weekly` (week list) + `/api/intelligence/weekly/[week]`. **Memories** = full-width memory browser (scope tabs, search) + edit-as-new-version; notes → `MemoryNoteDrawer`. Report/note links: `/visits/visit/<store>/<visit>?hl=<section>` → `VisitDrawer` (z 260; `hl` ∈ good_news|people_training|competitors|display_stock|follow_up highlights + scrolls that section; store name is a subtle ↗ link chaining to the store drawer; "🧠 Memories" section lists notes whose body references the visit), `/visits/store/<id>` → `StoreVisitDrawer`. Source: `app/page.tsx` (component `HomePage`, lifted from the old `/intelligence` page). NavBar tabs: **Reports · Statistics · Store Updates · Admin** (Admin tab admin-role only; Staff & Allies left the nav — reachable via the Admin page "More tools" row).
- `/statistics` — the analytics moved out of the old Overview: KPI cards (history drawers), Coverage heatmap (country/tier, cadence status), Execution (planned-vs-executed per CM), right detail drawer. 3-column layout mirroring `/visits`; deep-links into `/visits?store=|cm=|from=&to=`. Source: `app/statistics/page.tsx` (still carries some dead intel/memory state+helpers from the copy — pending `/simplify`).
- `/intelligence` — **redirects to `/`** (server `redirect("/")`). Kept only so existing Telegram broadcast + mini-app deep-links (`DASHBOARD_URL/intelligence`) still land on the report. See [[revamp-mockup-hides-deletion-surface]] for why the split happened instead of a delete.
...
- `/admin` — **Admin role only** (path-gated in `middleware.ts`, API routes use `requireAdmin`). Three cards: (1) **People** — pending join requests with inline market-pick Approve/Reject, manual Add Person form, active people table with editable role/market/AM and toggles for `is_intelligence_recipient` + `is_join_request_admin` (the two notification flags are decoupled from dashboard role — admins manage who-can-edit, toggles control who-gets-notified). (2) **Alert groups** — per-market chat_id + intelligence_mode (`people` / `group` / `both`) + Test button. (3) **Stores** — full CRUD with inline-editable cells, market/tier selects, deactivate/reactivate. Footer "More tools" row links to `/staff` + `/channel-managers` (both still live for am/cmic/admin, just out of the nav).
...
- `/api/stores/[id]/photos` — store photos grouped by ISO week (signed URLs, 8-week cap + `hasOlder`), feeds `StorePhotosDrawer`
