...
## Routes
- `/` — **Reports tab** (landing): the daily intelligence brief — exec stat-tiles + CM table + Signals/Alerts (parses stored `brief_markdown`), with day-nav (`‹ ›` arrows + date dropdown + Latest) and a greyed "Week" toggle (deferred AI weekly template). Memory browser (5 scope tabs, search) + edit-as-new-version kept below; store chips → visit drawer, notes → `MemoryNoteDrawer`. Source: `app/page.tsx` (component `HomePage`, lifted from the old `/intelligence` page). NavBar tabs: **Reports · Statistics · Store Updates · Staff & Allies**.
