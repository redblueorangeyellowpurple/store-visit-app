...
## Routes
- `/` — **Reports tab** (landing), three top-level tabs **Daily | Weekly | Memories**: **Daily** = the intelligence brief — exec stat-tiles + CM table + 🌟 Good News + Signals/Alerts + 🤝 Engagements cards (parses stored `brief_markdown`; new nested-bullet format AND legacy flat format both render), day-nav (`‹ ›` arrows + date dropdown + Latest + `?date=` deep-link). **Weekly** (`WeeklyView` + week-nav): deterministic sections (stat cards incl. Executed/Planned + engagement summary strip, Visits By Day, Execution Summary, nested-expandable Coverage By Tier + Display) computed live by `lib/weekly.ts`; AI narrative (Good News/Signals/Alerts/Engagements with kicker subtitles) read from `sva.weekly_reports` (mig 028) or a placeholder. Defaults to the last completed Mon–Sun week. Routes: `GET /api/intelligence/weekly` (week list) + `/api/intelligence/weekly/[week]`. **Memories** = full-width memory browser (scope tabs, search) + edit-as-new-version; notes → `MemoryNoteDrawer`. Report/note links: `/visits/visit/<store>/<visit>` → `VisitDrawer` (z 260, "Open store →" chains to store drawer), `/visits/store/<id>` → `StoreVisitDrawer`. Source: `app/page.tsx` (component `HomePage`, lifted from the old `/intelligence` page). NavBar tabs: **Reports · Statistics · Store Updates · Staff & Allies**.
